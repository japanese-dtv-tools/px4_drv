...
#### apt リポジトリを追加してインストール

apt リポジトリを追加してインストールすることも可能です。この場合、`apt update && apt upgrade` で自動的に新しいバージョンがインストールされます。
これは便利な反面、意図せずドライバが更新される可能性があるため、環境によっては不都合が生じる可能性があります。

以下のコマンドを実行して apt リポジトリを追加します。

```bash
curl -s --compressed "https://japanese-dtv-tools.github.io/px4_drv/KEY.gpg" | gpg --dearmor | sudo tee /etc/apt/trusted.gpg.d/px4_drv.gpg >/dev/null && \
 echo "deb [signed-by=/etc/apt/trusted.gpg.d/px4_drv.gpg] https://japanese-dtv-tools.github.io/px4_drv/ ./" | sudo tee /etc/apt/sources.list.d/px4_drv.list >/dev/null && \
 echo "success"
```

追加が終わったら success と表示されるので、次のコマンドでパッケージをインストールします。
...
バージョンを更新したい際は、以下のコマンドを実行してください。

```bash
sudo apt update && sudo apt upgrade px4-drv-dkms
```
